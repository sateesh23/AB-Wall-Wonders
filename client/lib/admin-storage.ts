...
export class AdminStorage {
  ...
  static getAllProjects(): ProjectData[] {
    ...
    try {
      ...
      return projectList.map((project: ProjectData) => ({
        ...
        images:
          project.images?.map((img) => ImageStorage.getImageUrl(img || "")) ||
          [],
      }));
    } catch (error) {
  ...
  static deleteProject(id: string): boolean {
    ...
    const referencedImages = filteredProjects
      .flatMap((p) => [p.thumbnail, p.image, ...(p.images || [])])
  ...
  static getStats() {
    ...
    return {
      ...
      flooringProjects: projects.filter((p) => p.category === "flooring")
        .length,
