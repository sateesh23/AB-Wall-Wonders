...
export const validateImageURL = (url: string): boolean => {
  ...
  try {
    ...
    if (
      urlObj.search &&
      /\.(jpg|jpeg|png|gif|bmp|webp|svg)/i.test(urlObj.search)
    ) {
      return true;
    }
...
export const createProject = async (
  projectData: Omit<FirebaseProject, "id" | "createdAt" | "updatedAt">,
): Promise<string> => {
  ...
  try {
    ...
    if (error.code === "permission-denied") {
      ...
      throw new Error(
        "Firestore database not found. Please ensure Firestore is enabled.",
      );
    } else {
