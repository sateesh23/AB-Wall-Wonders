import { supabase, type SupabaseProject, isSupabaseConfigured } from './supabase'
...
// Get recent projects
export const getRecentProjects = async (limitCount: number = 6): Promise<SupabaseProject[]> => {
  if (!isSupabaseConfigured()) {
    return []
  }
...
export const getProjectsByService = async (service: string): Promise<SupabaseProject[]> => {
  if (!isSupabaseConfigured()) {
    return []
  }
...
// Get project by ID
export const getProjectById = async (id: number): Promise<SupabaseProject | null> => {
  if (!isSupabaseConfigured()) {
    return null
  }
