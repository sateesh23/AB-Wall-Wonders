...
export class SanityService {
  ...
  private static convertToProjectData(sanityProject: SanityProject): ProjectData {
    return {
      ...
      thumbnail: sanityProject.image ? urlFor(sanityProject.image).width(400).height(300).url() : '/placeholder.svg',
      image: sanityProject.image ? urlFor(sanityProject.image).width(800).height(600).url() : '/placeholder.svg',
      images: sanityProject.images ? sanityProject.images.map(img => urlFor(img).width(800).height(600).url()) : [],
