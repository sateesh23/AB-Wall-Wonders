  ...
  static async createProject(data: CreateProjectData): Promise<string> {
    try {
      ...
      if (!validateImageURL(beforeImageURL)) {
        throw new Error(
          "Invalid before image URL. Please provide a valid image URL or leave empty for placeholder.",
        );
      }

      if (!validateImageURL(afterImageURL)) {
        throw new Error(
          "Invalid after image URL. Please provide a valid image URL or leave empty for placeholder.",
        );
      }
    ...
    try {
      ...
      if (data.beforeImageURL !== undefined) {
        if (data.beforeImageURL && !validateImageURL(data.beforeImageURL)) {
          throw new Error(
            "Invalid before image URL. Please provide a valid image URL.",
          );
        }
      ...
      if (data.afterImageURL !== undefined) {
        if (data.afterImageURL && !validateImageURL(data.afterImageURL)) {
          throw new Error(
            "Invalid after image URL. Please provide a valid image URL.",
          );
        }
  ...
  static async getAnalytics(): Promise<{
    ...
    try {
      ...
      const recentProjects = allProjects.filter(
        (project) => new Date(project.completedDate) > thirtyDaysAgo,
      ).length;
